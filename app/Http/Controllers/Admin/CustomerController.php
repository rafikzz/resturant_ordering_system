...
class CustomerController extends Controller
{
    ...
    {
        $title = $this->title;
        $customer =Customer::where('customer_type_id',1)->find($id);
    ...
    public function getType(Request $request)
    {
        if (true) {
            if ($request->customer_type == 2) {
                $customers = Customer::select('id', 'name', 'phone_no')->where('customer_type_id', $request->customer_type)->with('patient')->with('staff.department')->where('status', 1)->orderBy('name')->get();
            }else{
                $customers = Customer::select('id', 'name', 'phone_no')->where('customer_type_id', $request->customer_type)->with('patient')->with('staff')->where('status', 1)->orderBy('name')->get();
    ...
    {
        if ($request->ajax()) {
            $data = Customer::select('table_customers.*')->where('customer_type_id',1)->withCount(['orders as orders_total'=>function($query) {
