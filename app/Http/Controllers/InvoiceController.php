...
class InvoiceController extends Controller
{
    ...
    {
        // Checked Print Order if not checked out returns 1
        $noCheckoutFailed =  PrintOrderService::printBill($id);
        if ($noCheckoutFailed) {
            return redirect()->back()->with('error', 'Please Checkout The Order First');
        }
    }

    function printKot(Request $request)
    {
        if($request->ajax())
        {
            if($request->order_id)
            {
                PrintOrderService::printKot($request->order_id, $request->order_no);
